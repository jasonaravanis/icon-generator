import { breakpoints, getQuery } from '@style-system/core/breakpoints.css';
...
export const responsiveStyle = <T>(styles: Partial<Record<Breakpoint, T>>): Result<T> => ({
  '@media': Object.entries<T>(styles).reduce<Record<string, T>>((acc, [key, value]) => {
